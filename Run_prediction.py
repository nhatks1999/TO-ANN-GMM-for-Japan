...
feature_cols = ["fnet_MT_Magnitude(Mw)", "Event depth", "ET Interface", "ET Crustal", "ET Slab",
    "FM Normal", "FM Reverse", "FM Strike-slip", "FM Unknown", "Region", "Rrup", "CS", "VS30"]

onehot_cols = ["ET Interface", "ET Crustal", "ET Slab", "FM Normal", "FM Reverse", "FM Strike-slip", "FM Unknown"]
binary_cols = ["Region"]
cat_cols = onehot_cols + binary_cols

num_cols = [c for c in feature_cols if c not in cat_cols]
cat_idx = [feature_cols.index(c) for c in cat_cols]
num_idx = [feature_cols.index(c) for c in num_cols]

RENAME_MAP = {"Mw": "fnet_MT_Magnitude(Mw)"}

df_new = pd.read_csv(INPUT_NEW_DATA)
df_new = df_new.rename(columns=RENAME_MAP)

X_new_raw = df_new[feature_cols].astype("float32")
...
for fold in range(1, NUM_FOLDS + 1):
    ...
    mean = scaler_df.loc[scaler_df["Stat"] == "mean", num_cols].iloc[0].values.astype(float)
    scale = scaler_df.loc[scaler_df["Stat"] == "scale", num_cols].iloc[0].values.astype(float)
...
df_final = pd.DataFrame({"Motion ID": motion_ids})
